...
function renderMonthCalendarView() {
    ...
    allDays.forEach(day => {
        ...
        div.className = `p-0.5 sm:p-2 border border-slate-100 rounded-md sm:rounded-xl min-h-[55px] sm:min-h-[90px] flex flex-col justify-between cursor-pointer transition text-left ${day.isCurrentMonth ? 'bg-white hover:border-theme-orange' : 'bg-slate-50/70 opacity-40'}`;
        ...
        if(log) {
            ...
            if (log.schedules && log.schedules.length > 0) badgeRows += `<span class="px-0.5 sm:px-1 text-[7px] sm:text-[9px] bg-indigo-100 text-indigo-700 font-black rounded truncate">📌${log.schedules.length}</span>`;
        ...
        div.innerHTML = `
            <span class="text-[10px] sm:text-xs font-black text-slate-700">${day.dayNum}</span>
...
function renderWeekCalendarView() {
    ...
    targetWeekDays.forEach(day => {
        ...
        const schedules = log ? log.schedules || [] : [];
        ...
        schedules.forEach(s => listHtml += `<div class="bg-indigo-50 border-l-2 border-indigo-500 text-indigo-800 ${itemBase}">📌 ${s}</div>`);
        ...
        const div = document.createElement('div');
        div.className = "p-1 sm:p-3 min-h-[200px] sm:min-h-[300px] space-y-1 sm:space-y-2 bg-white cursor-pointer hover:bg-slate-50/50 text-left overflow-hidden";
        ...
        div.innerHTML = `
            <div class="border-b pb-0.5 sm:pb-1 flex justify-between items-center">
                <span class="text-[9px] sm:text-xs font-black text-slate-800 truncate"><span class="sm:hidden">${day.dayNum}</span><span class="hidden sm:inline">${day.dayNum}일 (${day.fullDateStr.slice(5)})</span></span>
            </div>
